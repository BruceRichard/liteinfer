...
import torch
from liteinfer.distributed import DistributedInfo
from liteinfer.scheduler import SchedulerConfig
from liteinfer.utils import init_logger
...
class ServerArgs(SchedulerConfig):
    ...
    def zmq_frontend_addr(self) -> str:
        return "ipc:///tmp/liteinfer_3" + self._unique_suffix

    @property
    def zmq_tokenizer_addr(self) -> str:
        if self.share_tokenizer:
            return self.zmq_detokenizer_addr
        result = "ipc:///tmp/liteinfer_4" + self._unique_suffix
...
def parse_args(args: List[str], run_shell: bool = False) -> Tuple[ServerArgs, bool]:
    ...
    """
    from liteinfer.attention import validate_attn_backend
    from liteinfer.kvcache import SUPPORTED_CACHE_MANAGER
    from liteinfer.moe import SUPPORTED_MOE_BACKENDS
    ...
    if (dtype_str := kwargs["dtype"]) == "auto":
        from liteinfer.utils import cached_load_hf_config
